...
// ดึงข้อมูลสินค้าที่ผู้ขายลงขายจากฐานข้อมูล
...
if (isset($_POST['delete'])) {
    ...
    // ดึงข้อมูลสินค้าก่อนลบเพื่อจัดการรูปภาพ
    ...
    if ($product && $product['Image']) {
        ...
        foreach ($images as $image) {
            if (file_exists($image)) {
                unlink($image);  // ลบไฟล์รูปภาพจากเซิร์ฟเวอร์
            }
        }
    }
    ...
    if ($delete_stmt->execute()) {
        echo "สินค้าถูกลบเรียบร้อยแล้ว!";
        header("Location: seller.php");  // รีเฟรชหน้าเพื่อแสดงการอัปเดต
        exit;
    } else {
        echo "เกิดข้อผิดพลาดในการลบสินค้า: " . $delete_stmt->error;
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <!-- แสดงสินค้าที่ผู้ขายลงขาย -->
    <div class="products">
        <?php
        if ($result->num_rows > 0) {
            // ลูปผ่านข้อมูลสินค้าและแสดง
            while ($product = $result->fetch_assoc()) {
                // แยกรูปภาพที่อัปโหลดหลายๆ รูปจากฐานข้อมูล (ใช้คั่นด้วยเครื่องหมาย comma)
                $images = explode(',', $product['Image']);
                // หากมีรูปภาพหลายรูปให้เอารูปแรกมาแสดง
                $first_image = $images[0] ?? 'images/default.jpg'; // ใช้รูปแรก หรือใช้รูปดีฟอลต์หากไม่มี

                echo '<div class="product">';
                
                // แสดงรูปภาพพร้อมลิงก์ไปยังหน้าเกี่ยวกับคำสั่งซื้อของสินค้า
                echo '<a href="edit_product.php?product_id=' . htmlspecialchars($product['ProductID']) . '">';  // ส่ง product_id ไปที่หน้า edit_product.php
                echo '<img src="' . htmlspecialchars($first_image) . '" alt="' . htmlspecialchars($product['ProductName']) . '">';
                echo '</a>';

                // แสดงชื่อสินค้า ราคา และคำสั่งซื้อ
